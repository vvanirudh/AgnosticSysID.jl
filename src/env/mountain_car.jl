...
const start_position_noise = 0.0
...
function MountainCar(rock_c::Float64; position_sigma::Float64 = 0.0, seed::Int64 = 0)
    start_state = MountainCarState(-π / 6, 0)
    position_discretization = 1000
    speed_discretization = 1000
    min_position = -1.2
    max_position = 0.5
    max_speed = 0.07
    goal_position = 0.5
...
function init(mountaincar::MountainCar; cont = false)
    start_state = mountaincar.start_state
    if start_position_noise > 0.0
        random_position = rand(
            mountaincar.rng,
            Uniform(
                mountaincar.start_state.position - start_position_noise,
                mountaincar.start_state.position + start_position_noise,
            ),
        )
        start_state = MountainCarState(random_position, mountaincar.start_state.speed)
    end
    if cont
        return start_state
    end
...
function position_dynamics(mountaincar::MountainCar, state::MountainCarState)
    ...
    new_position += rand(mountaincar.rng, mountaincar.noise)
